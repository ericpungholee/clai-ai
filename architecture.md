...
| `backend/app/providers/` | fal transport and Nano Banana Pro, FLUX Fill, SAM, TRELLIS multi-image, and Tripo adapters. |
...
An existing attempt or nonfailed cache is reused. A completed textureless mesh may be upgraded to `standard`, and a historical single-view fallback may be upgraded after a complete angle set becomes available; failed attempts can be explicitly retried with a new attempt ID. Upgrades reuse/reset the cache row, so there is no separate durable history of previous mesh attempts.

The primary image transaction creates three queued `VersionImageView` rows keyed by `(version_id, angle)` for right/back/left, alongside the immutable `Version`. After committing, the worker dispatches three independent [angle tasks](backend/app/services/image_view_jobs.py). Nano Banana Pro uses that version's primary artifact as its only image input, includes the frozen design prompt, and requests one camera-specific output per call. Artifacts use the existing validated ingestion/storage path. Each task claims its queued row once; errors and enqueue/setup failures mark only that angle failed. The canvas keeps reading `Version.artifact_url`, but API run status remains `ingesting` until all three angle rows finish and then reports complete or failed. The version API includes a `views` map with each angle's URL and status; the existing graph poll updates the canvas and open viewers by version ID.

[MeshProvider](backend/app/providers/mesh.py) uploads completed artifacts separately and calls [fal-ai/trellis-2/multi](https://fal.ai/api/openapi/queue/openapi.json?endpoint_id=fal-ai/trellis-2/multi) with native `image_urls: [front, right, back, left]`. New multi-angle versions cannot queue a mesh until all three auxiliary rows contain artifacts. Multi-angle requests use the demo `high_quality` preset values: resolution 1536, texture size 4096, and 12 sampling steps for structure, shape, and texture. Both the API and worker require exactly four distinct URLs and forbid singular `image_url`; partial sets cannot reach fal. The root TRELLIS-2 route only accepts single-image input. The mesh API freezes the ordered URL list in `VersionMesh.source_image_urls` when queuing an attempt. New attempts upgrade completed caches with outdated models or quality settings. Historical versions with no angle rows retain the existing [TripoProvider](backend/app/providers/tripo.py) single-image path. The [mesh worker](backend/app/services/mesh_jobs.py) records the provider output before validation and storage, allowing ingestion retries without another paid generation. It accepts TRELLIS `model_glb` files with embedded standard or WebP-extension textures and no preview. Mesh failures leave the 2D version unchanged.

Migration `d2b9a71f403e` expands the prior rear-image table, preserves existing artifacts as angle `back`, and migrates frozen primary/rear mesh URLs to the ordered JSON list. Old versions are not backfilled. This demo implementation does not automatically recover angle tasks after worker termination; normally caught provider and dispatch failures are terminal and visible. Model prompting cannot guarantee exact hidden geometry or graphics, and multi-view mesh quality needs visual evaluation.
