...
An image run generates and stores one main image. The worker commits the Version and completes its RunJob immediately after ingestion. No supporting image jobs are submitted. The Version's artifact URL is the sole reconstruction source, including for older versions containing legacy metadata.

[TrellisProvider](backend/app/providers/trellis.py) uploads that stored image unchanged to `fal-ai/trellis-2` with a singular `image_url`. There is no category prompt or additional image-generation stage. Geometry resolution is 1024, texture size 2048, target vertices 50,000, 12 sampling steps per stage, and remesh is disabled. The exact submitted payload, request ID, elapsed time and output hashes remain recorded in VersionMesh. See [the single-image flow](docs/image-to-3d-preview.md).
...
The viewer displays the provider mesh and embedded textures directly, without automatic logo extraction or decal controls. The existing local decal utilities and legacy logo API are isolated from reconstruction and viewing.

`MeshViewer` polls two seconds after the preceding request finishes and dynamically loads the Three.js scene. OrbitControls permits 360° rotation and top/bottom inspection; Reset view restores the front camera. Closing the modal stops polling while worker execution continues. Export GLB downloads the stored provider artifact. The model is an inferred visualization, not editable CAD geometry.
